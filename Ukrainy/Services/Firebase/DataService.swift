...
class DataService {
        ...
        ImageManager.instance.uploadPostImage(postID: postID, image: image) { (success) in
            ...
            if success {
                ...
                document.setData(postData) { (error) in
                    ...
                    if let error = error {
                        print("📕: Error uploading data to post document! Description: \(error.localizedDescription)")
                        handler(false)
                        return
                    } else {
                        handler(true)
    ...
    func uploadReport(reason: String, postID: String, handler: @escaping (_ success: Bool) -> ()) {
        ...
        REF_REPORTS.addDocument(data: data) { (error) in
            ...
            if let error = error {
                print("📕: Error uploading report! Description: \(error.localizedDescription)")
                handler(false)
                return
            } else {
                handler(true)
    ...
    func uploadComment(postID: String, content: String, displayName: String, userID: String, handler: @escaping (_ success: Bool, _ commentID: String?) -> ()) {
        ...
        document.setData(data) { (error) in
            ...
            if let error = error {
                print("📕: Error uploading comment! Description: \(error.localizedDescription)")
                handler(false, nil)
                return
            } else {
